      ...
      <div className="absolute inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl flex flex-col transform transition-transform duration-500">
            ...
            <div className="space-y-6">
              {(items || []).map((item) => (
                <div key={item.id} className="flex gap-4">
                  <div className="w-24 h-24 bg-slate-100 rounded-2xl overflow-hidden"><img src={item.image || undefined} alt={item.name} className="w-full h-full object-cover" /></div>
                  <div className="flex-grow">
                    ...
                    <p className="text-indigo-600 font-bold mt-1">{currency}{(Number(item.price) || 0).toFixed(2)}</p>
        ...
        {(items || []).length > 0 && (
          <div className="p-6 border-t border-slate-100 space-y-4 bg-slate-50/50">
            <div className="flex justify-between items-center text-lg"><span className="text-slate-500">{t.subtotal}</span><span className="font-bold text-slate-900">{currency}{(Number(total) || 0).toFixed(2)}</span></div>
