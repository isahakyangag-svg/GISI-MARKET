              ...
              {!user.isPhoneVerified && !isVerifying && (
                <div className="bg-slate-50 p-6 rounded-[1.8rem] border border-slate-100 space-y-4">
                  ...
                  <button 
                    ...
                    disabled={isLoading}
                    className="w-full py-3 bg-[#3BB19B] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-transform disabled:opacity-50"
                  >
                    {isLoading ? 'Отправка...' : 'Подтвердить по СМС'}
                  </button>
                </div>
              )}
              ...
              {isVerifying && (
                <div className="bg-emerald-50 p-6 rounded-[1.8rem] border border-emerald-100 space-y-4 animate-in fade-in slide-in-from-top-4">
                   ...
                   <input 
                      ...
                      className="w-full p-4 bg-white border border-emerald-200 rounded-2xl text-center text-xl font-black outline-none tracking-[0.5em] focus:border-emerald-400 transition-colors"
                   />
                   
                   <div className="flex flex-col gap-4">
                      <div className="grid grid-cols-2 gap-3">
                        <button onClick={handleVerifyCode} className="py-3 bg-[#3BB19B] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md">OK</button>
                        <button onClick={() => setIsVerifying(false)} className="py-3 bg-white text-slate-400 border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest">Отмена</button>
                      </div>
                      
                      <button 
                        onClick={handleResendCode}
                        disabled={resendTimer > 0}
                        className={`text-[9px] font-black uppercase tracking-[0.2em] text-center transition-colors ${resendTimer > 0 ? 'text-slate-400' : 'text-indigo-600 hover:text-indigo-800'}`}
                      >
                        {resendTimer > 0 ? `Отправить снова через ${resendTimer}с` : 'Отправить код повторно'}
                      </button>
                   </div>
                </div>
              )}
