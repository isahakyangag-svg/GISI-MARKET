...
export const CartPage: React.FC<CartPageProps> = ({
  ...
  const subtotal = useMemo(() => (items || []).reduce((acc, item) => acc + (Number(item.price) || 0) * item.quantity, 0), [items]);
  const originalSubtotal = useMemo(() => (items || []).reduce((acc, item) => acc + (Number(item.oldPrice || item.price) || 0) * item.quantity, 0), [items]);
                ...
                {(items || []).map((item) => (
                  <div key={item.id} className="py-8 flex flex-col md:flex-row gap-8 items-center">
                     <div className="w-32 h-32 bg-slate-50 rounded-[2rem] p-4 flex items-center justify-center shrink-0 border border-slate-100/50">
                        <img src={item.image || undefined} alt={item.name} className="max-h-full max-w-full object-contain mix-blend-multiply" />
                     </div>
                     ...
                     <div className="flex flex-col md:items-end gap-4 shrink-0 w-full md:w-auto">
                        <div className="flex items-center gap-6 justify-center">
                           ...
                           <p className="text-xl font-black text-slate-950">{((Number(item.price) || 0) * item.quantity).toLocaleString()} {settings?.currency || '֏'}</p>
                        </div>
